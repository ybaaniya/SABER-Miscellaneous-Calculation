...
main_curves_df = pd.read_csv('/path to cluster center obtained after the clustering/cluster_centers_5.csv')
main_curves_df = main_curves_df.sort_values(by='Unnamed: 0')
df = main_curves_df.drop(columns=['Unnamed: 0'])
...
output_file = 'path to save the closest cluster center along with its distance.csv' #3 column csv with file name, closest cluster center and distance
